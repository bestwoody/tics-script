...
>> create table test (
	...
	a Int32,
	b Int32
	) engine = MutableMergeTree(dt, (dt), 8192)
...
Code: 69. DB::Exception: Received from {#WORD}, {#WORD} DB::Exception: Internal column name can not be dropped: _INTERNAL_VERSION.

>> alter table test add column _INTERNAL_DELMARK Int32
Received exception from server (version {#WORD}):
Code: 69. DB::Exception: Received from {#WORD}, {#WORD} DB::Exception: Internal column name can not be used: _INTERNAL_DELMARK.

>> alter table test modify column _INTERNAL_VERSION Int64 default 6
Received exception from server (version {#WORD}):
Code: 69. DB::Exception: Received from {#WORD}, {#WORD} DB::Exception: Internal column name can not be modified: _INTERNAL_VERSION.
