...
void ReplacingFinalBlockInputStream::insertRow(MutableColumns & merged_columns, size_t & merged_rows)
{
...
Block ReplacingFinalBlockInputStream::readImpl()
{
    if (finished)
        return Block();

    // Final merge can't skip it
    // if (children.size() == 1)
...
void ReplacingFinalBlockInputStream::merge(MutableColumns & merged_columns, std::priority_queue<SortCursor> & queue)
{
