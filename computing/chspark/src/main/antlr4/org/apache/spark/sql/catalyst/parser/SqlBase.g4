...
statement
    ...
    | CREATE FLASH? DATABASE (IF NOT EXISTS)? identifier
    ...
    | CREATE TABLE (IF NOT EXISTS)? FROM TIDB
        source=tableIdentifier tableProvider                           #createTableFromTiDB
    ...
    | LOAD DATA FROM TIDB OVERWRITE? INTO TABLE tableIdentifier        #loadDataFromTiDB
...
tableProvider
    : USING (chTableProvider | qualifiedName )
    ;
...
colType
    : identifier dataType (primaryKey)? (notNull)? (COMMENT STRING)?
    ;
