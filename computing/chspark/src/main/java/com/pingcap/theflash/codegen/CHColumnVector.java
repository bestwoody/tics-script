...
public class CHColumnVector extends ColumnVector {
  ...
  public float getFloat(int rowId) {
    if (type == CHTypeNumber.CHTypeFloat64.instance) {
      // In some case like min(tp_int8 + tpfloat32 * 2), CH cast the result as Float64 while Spark
      // not.
  ...
  public Decimal getDecimal(int rowId, int precision, int scale) {
    if (type == CHTypeNumber.CHTypeUInt64.instance) {
      ...
      if (v >= 0) {
        ...
        // return new Decimal().set(BigDecimal.exact(new
        // java.math.BigDecimal(toUnsignedBigInteger(v))));
      }
      ...
      if (result.changePrecision(precision, scale)) {
        ...
        throw new IllegalArgumentException(
            "Decimal(" + precision + ", " + scale + ") value " + result + " overflowed.");
      }
    }
