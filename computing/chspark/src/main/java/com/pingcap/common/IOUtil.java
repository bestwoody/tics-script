...
public class IOUtil {
  ...
  public static long readVarUInt64(ReadableByteChannel in, ByteBuffer oneByteBuf)
      throws IOException {
    long value = 0;
    int i = 0;
    long b;
  ...
  public static void writeUTF8StrWithVarLen(String string, WriteBuffer writer, ByteBuffer strBuf_)
      throws IOException {
    try {
      byte[] bytes = string.getBytes("UTF-8");

      ByteBuffer strBuf =
          bytes.length > strBuf_.capacity() ? MemoryUtil.allocateDirect(bytes.length) : strBuf_;
      strBuf.clear();
      strBuf.put(bytes);
      strBuf.flip();
  ...
  public static String readUTF8StrWithVarLen(
      ReadableByteChannel in, ByteBuffer oneByteBuf, ByteBuffer strBuf_) throws IOException {
  ...
  public static String readUTF8StrWithVarLen(ReadBuffer reader, ByteBuffer strBuf_)
      throws IOException {
  ...
  public static byte[] readBytesWithVarLen(ReadBuffer reader, ByteBuffer strBuf_)
      throws IOException {
    long len = readVarUInt64(reader);
    if (len > DEFAULT_MAX_STRING_SIZE) {
      throw new IOException("Too large string size.");
    ...
    strBuf.limit(strLen);
    reader.read(strBuf);
    byte[] bytes = UTF8String.fromAddress(null, MemoryUtil.getAddress(strBuf), strLen).getBytes();
    if (strBuf != strBuf_) {
      MemoryUtil.free(strBuf);
    }
    return bytes;
