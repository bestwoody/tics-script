...
public class CHColumnString extends CHColumn {
  ...
  /// Maps i'th position to offset to i+1'th element. Last offset maps to the end of all chars (is
  // the size of all chars).
  private ByteBuffer offsets; // UInt64
  /// Bytes of strings, placed contiguously.
  /// For convenience, every string ends with terminating zero byte. Note that strings could contain
