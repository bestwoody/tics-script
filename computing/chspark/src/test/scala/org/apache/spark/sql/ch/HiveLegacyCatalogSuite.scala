...
class HiveLegacyCatalogSuite extends BaseLegacyCatalogSuite {
  ...
  override def verifyShowDatabases(expected: Array[String]): Unit = {
  ...
  override def verifyShowTables(db: String,
    ...
    if (db == "default") {
      // Default may have legacy tables, use another way to compare.
      expected.forall(r.contains)
    } else {
    ...
    if (otherDb == "default") {
      // Default may have legacy tables, use another way to compare.
      otherExpected.forall(r.contains)
    } else {
