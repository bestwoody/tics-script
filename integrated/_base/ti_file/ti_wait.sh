...
function set_cluster_version()
{
	if [ -z "${3+x}" ]; then
		echo "[func set_cluster_version] usage: <func> host port bins_dir [server_id]" >&2
		return 1
	fi
	...
	local pd_ctl_path="${bins_dir}/pd-ctl"
	if [ ! -f "${pd_ctl_path}" ]; then
		echo "[func set_cluster_version] '${pd_ctl_path}' not found" >&2
		return 1
	fi
	...
	for ((i=0; i<10; i++)); do
		...
		local cluster_version=`"${pd_ctl_path}" -u "http://${host}:${port}" <<< "config show all" | { grep "cluster-version" || test $? = 1; } | awk '{print $2}' | tr -d '\"'`
		if [ "${cluster_version}" == "3.0.0-alpha" ]; then
			break
		fi
		sleep 1
	done
...
export -f set_cluster_version

function set_cluster_version_local()
{
	if [ -z "${1+x}" ]; then
		echo "[func set_cluster_version_local] usage: <func> pd_dir" >&2
		return 1
	fi
	...
	local pd_info="${pd_dir}/proc.info"
	if [ ! -f "${pd_info}" ]; then
		echo "[func set_cluster_version_local] '${pd_dir}' pd not exists" >&2
		return 1
	fi
	local host=`get_value "${pd_info}" 'advertise_host'`
	local port=`get_value "${pd_info}" 'pd_port'`
	...
	set_cluster_version "${host}" "${port}" "${pd_dir}" "${server_id}"
}
export -f set_cluster_version_local

function set_cluster_version_by_host()
{
	if [ -z "${4+x}" ]; then
		echo "[func set_cluster_version_by_host] usage: <func> host port bins_dir default_ports_file" >&2
		return 1
	fi
	...
	set_cluster_version "${host}" "${port}" "${bins_dir}"
}
export -f set_cluster_version_by_host
