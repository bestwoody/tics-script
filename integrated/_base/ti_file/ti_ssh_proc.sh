...
{
	if [ -z "${7+x}" ]; then
		echo "[func ssh_prepare_run] usage: <func> host mod_name dir conf_templ_dir cache_dir remote_env version" >&2
		return 1
	fi
	...
	# TODO: Tidy up these paths
	cp_bin_to_host "${name}" "${host}" "${remote_env}" "${cache_dir}/worker/bins" "${bin_paths_file}" "${bin_urls_file}" "${cache_dir}/master/bins" "${version}"
