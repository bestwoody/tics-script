...
function pd_run()
{
	...
	if [ -z "${initial_cluster}" ]; then
		initial_cluster="${pd_name}=http://${advertise_host}:${peer_port}"
	else
	...
	"${pd_dir}/pd-server" \
		...
		--log-file="${pd_dir}/pd.log" 2>> "${pd_dir}/pd_stderr.log" &
...
function tikv_run()
{
	...
	"${tikv_dir}/tikv-server" \
		...
		--log-file "${tikv_dir}/tikv.log" 2>> "${tikv_dir}/tikv_stderr.log" &
...
function tidb_run()
{
	...
	"${tidb_dir}/tidb-server" \
		...
		--log-file="${tidb_dir}/tidb.log" 2>> "${tidb_dir}/tidb_stderr.log" &
...
function rngine_run()
{
	...
	"${rngine_dir}/tikv-server-rngine" \
		...
		--log-level info \
		--log-file "${rngine_dir}/rngine.log" 2>> "${rngine_dir}/rngine_stderr.log" &
