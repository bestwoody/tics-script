...
function get_learner_id_for_region()
{
	...
	echo `"${integrated}/ops/ti.sh" -i 0 "${test_ti_file}" 'pd_ctl' "region ${region_id}" | { grep -B 1 "is_learner" || test $? = 1; } | { grep "store_id" || test $? = 1; } | awk '{print $2}' | tr -d ','`
}
...
function transfer_learner()
{
	...
	"${ti}" -i 0 "${test_ti_file}" 'pd_ctl' "operator add add-learner ${region_id} ${learner_id}"
	for ((k=0; k<${timeout}; k++)); do
		echo "region id ${region_id}"
		echo "target learner id ${learner_id}"
		local learner_count=`"${ti}" -i 0 "${test_ti_file}" 'pd_ctl' "region ${region_id}" | { grep "is_learner" || test $? = 1; } | wc -l`
...
function ti_manual_schedule()
{
	...
	for ((i=0; i<${cluster_region_count}; i++)); do
		local region_info=`"${ti}" "${test_ti_file}" 'pd_ctl' "region ${i}"`
