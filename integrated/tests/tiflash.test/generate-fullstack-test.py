...
def run():
    if len(sys.argv) != 4:
        print 'usage: <bin> database table test_target_dir'
        sys.exit(1)
    ...
    directory = os.path.dirname(test_target_dir)
    try:
        os.makedirs(directory)
    except OSError as e:
        if e.errno != errno.EEXIST:
            raise
    generate_cases(database, table, types, data, primary_key_candidates, primary_key_data, dml_test_cases, test_target_dir)
